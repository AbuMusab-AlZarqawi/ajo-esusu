...
export default function PoolCard({ poolId, userAddress, onJoinSuccess }: PoolCardProps) {
  ...
  const poolStatus = Number(status);
  const isCompleted = poolStatus === 1 || poolStatus === 2; // 1=Completed, 2=Cancelled only
  const isActive = poolStatus === 0;
  ...
  return (
    ...
    >
      ...
      {isCompleted && (
        <div className="w-full py-3 text-center text-white/30 text-sm border border-white/10 rounded-xl">
          {poolStatus === 1 ? "Pool Completed ✓" : "Pool Cancelled"}
        </div>
      )}
    </motion.div>
  );
