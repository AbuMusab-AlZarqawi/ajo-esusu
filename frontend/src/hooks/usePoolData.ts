...
export function getNextCycleTime(lastTimestamp: number, frequency: number): string {
  ...
  // Bad/zero timestamp — pool just started, cycle begins now
  if (!lastTimestamp || lastTimestamp < 1000000) return "Starting soon";
  ...
  // Guard: if > 365 days, timestamp is bad data
  if (diff > 365 * 24 * 3600) return "Due now";
